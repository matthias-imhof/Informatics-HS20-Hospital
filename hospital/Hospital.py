        ...
        for i in self._employees:
            if isinstance(i, Doctors):
                if patient._problem.lower() == i.can_treat.lower():
                    ...
                    i.treat(patient)
                    return "Patient's problem: {} could be treated and the patient: {} {} is healthy again :)".format(
                        temp, patient._firstname, patient._lastname
                    )
        if len(patient._problem) > 0:
            return "Patient: {} {} couldn't be treated as there is no doctor in the {} hospital to treat: {}".format(
                patient._firstname, patient._lastname, self._Hospitalname, patient._problem
            )
        ...
        if isinstance(employee, Employees):
            for i in self._employees:
                if i == employee:
                    ...
                    return "Successfully removed {} the employee from the {} Hospital".format(
                        employee, self._Hospitalname
                    )
        else:
...
class Doctors(Employees, ABC):
    ...
    def _works(self):
        return "The: {} name is: {} {} and has the following titles: {}".format(
            type(self).__name__, self._firstname, self._lastname, self._title
        )
    ...
    def __repr__(self):
        return "{} {} {} {} {} {}".format(
            type(self).__name__, self._firstname, self._lastname, self.salary, self._title, self.can_treat
        )
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.can_treat == other.can_treat
        )
